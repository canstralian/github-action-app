...
- Cancel superseded runs with a workflow-level concurrency group
...
Run scheduled and pull-request scans to detect vulnerabilities early. Use the `javascript-typescript` CodeQL language setting with `github/codeql-action/init@v3` so the workflow matches the current JavaScript/TypeScript analyzer configuration.
